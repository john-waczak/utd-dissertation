...
% - Hysperspectral imaging is an increasingly popular tool
% - Popular remote sensing platforms currently employ multi-spectral sensors with
% limited spatial/spectral/temporal resolutions
% - Many missions incorporating HSI have been recently launched or will soon be
% launched
% - Still for practical applications, one must utilize in-situ data to calibrate
% what's measured by HSI to get relevant physical parameters; what we measure is
% not exactly what we want
% - This can require decades of data since you need cloud-free, serendipetous
% satellite overpasses
% - Satellite imagery must also be corrected for atmospheric effects including
% absorption by gases, particulate scattering, and more
% - The shrinking size of HSI technology makes it possible to incorporate them
% into modern drones (quadcopters, etc.) which can be readily deployed directly to
% sensing sites
% - combining this with autonomous in-situ data collection using other sensing
% sentinels can dramatically increase collected data volumes in a fraction of the time.
